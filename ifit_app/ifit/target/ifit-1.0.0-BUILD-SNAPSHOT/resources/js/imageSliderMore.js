	...
	$.ajax({
		...
		success: function(obj){
			...
			for(var i in photoList){
				if(photoList[i].bannerType==1){
					var htmlCode = '<div data-p="225.00" style="display: none;">';
					htmlCode+= '<img data-u="image" src="http://'+photoList[i].bannerURL+'" /></div>';
					$("#jssor_1_container").append(htmlCode);
				}
				if(photoList[i].bannerType==2){
					var htmlCode = '<img src="http://'+photoList[i].bannerURL+'">';
					$("#sampleMoreImgs").append(htmlCode);
				}
			}
		}
	});
