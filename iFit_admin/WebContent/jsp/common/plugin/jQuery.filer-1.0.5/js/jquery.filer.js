...
//								.slice(-2) != "[]" ? s.attr("name", s.attr("name") + "[]") : null;
	...
	$.fn.filer.defaults = {
		...
		templates: {
			...
			item: '<li class="jFiler-item"><div class="jFiler-item-container"><div class="jFiler-item-inner"><div class="jFiler-item-icon pull-left">{{fi-icon}}</div><div class="jFiler-item-info pull-left"><div class="jFiler-item-title" title="{{fi-name}}">{{fi-name | limitTo:30}}</div><div class="jFiler-item-others"><span>크기: {{fi-size2}}</span><span>형식: {{fi-extension}}</span><span class="jFiler-item-status">{{fi-progressBar}}</span></div><div class="jFiler-item-assets"><ul class="list-inline"><li><a class="icon-jfi-trash jFiler-item-trash-action"></a></li></ul></div></div></div></div></li>',
		...
		captions: {
			...
			feedback2: "개의 파일이 선택 되었습니다.",
			drop: "Drop file here to Upload",
			removeConfirmation: "파일을 삭제 하시겠습니까?",
			errors: {
				filesLimit: "최대 {{fi-limit}}개의 파일만 등록 가능합니다.",
				filesType: "다음과 같은 파일 형식만 첨부 가능합니다. [{{fi-extensions}}]",
